...
static void choose_t(ge25519_aff *t, unsigned long long pos, signed char b)
{
  ...
  *t = ge25519_base_multiples_affine[5*pos+0];
  ...
  //luca: cmov updates 32*uint32_t in t->x and t->y with second param depending on third arg
  //cmov applies following to x and y coords
  // int i;
  ...
  fe25519_neg(&v, &t->x); //writes to &v, reads from t->x; does some arithmetic
...
//luca: secand param is secret, first param is result
//sc25519 is as uint32_t[32] array
void ge25519_scalarmult_base(ge25519_p3 *r, const sc25519 *s)
{
  signed char b[85];
  int i;
  ge25519_aff t; //luca: we need to track this for changes!
  /*
 Do not add prints here, as this will change the layout of the code!!!
*/


  

  //luca:: copies (parts) of s into b, applying weird
...
void compute_r_from_sc_info(signed char* b, uint8_t * r_out ) {
    ge25519_p3* r = alloca(sizeof(ge25519));
    int i;
    ge25519_aff t;
      ...
      ge25519_pack(r_out, r);

}

void compute_r_unsigned(uint8_t * secret ,int8_t* r) {
    sc25519* s = alloca(sizeof(sc25519));
    sc25519_from64bytes(s, secret);
    char carry;
      int i;
      for(i=0;i<10;i++)
      {
